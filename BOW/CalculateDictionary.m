...
%     inFName = fullfile(image_dir, sprintf('%s', featureName));
    inFName = strcat(opts.localdatapath,'\',num2str(1),'_sift');
    load(inFName, 'features');
    data = features.data;
    
%     image_dir=sprintf('%s/%s/',opts.localdatapath,num2string(2,4)); % location descriptor
%     inFName = fullfile(image_dir, sprintf('%s', 'sift_features'));
    inFName = strcat(opts.localdatapath,'\',num2str(2),'_sift');
    load(inFName, 'features');
 ...
 %           image_dir=sprintf('%s/',opts.localdatapath); % location descriptor
 %           inFName = fullfile(opts.localdatapath, sprintf('%s',featureName));
            inFName = strcat(opts.localdatapath,'\',num2str(f),'_sift');
